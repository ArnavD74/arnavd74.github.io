...
function update() {
	...
	//succ circle
	ctx.strokeStyle =
		"rgba(255, 255, 255, " + ((3 / pullLength) * 1) / attractionTimer + ")";
...
function dot() {
	...
	this.t =
		minTransparency + Math.random() * (maxTransparency - minTransparency);
	...
	this.update = function () {
		...
		if (attractionTimer <= 0) {
			...
			var angle = getAngle(this.x, this.y, mx, my);
			var force =
				(pullFactor / Math.pow(dist(this.x, this.y, mx, my), 2)) *
				(attractionTimer + pullLength);
				...
				if (distance <= connectionDistance + fadeOut) {
					var transparency =
						this.t *
							...
							sigmoid(
								(-6 / (2 * fadeOut)) *
									(distance - (connectionDistance + fadeOut))
							) -
						1;
					ctx.strokeStyle =
